...
if( isset( $_POST['submit'] ) && $_POST['submit'] != "" ) {
	...
	foreach( $file_names as $file_name ) {
		...
		if( is_readable( $file_path ) && in_array( strtolower( $ext ) , $array_allowed_exts ) ) {
			...
			ielog_create_thumb( "$photos_dir/$lid.$ext" , $lid , $ext ) ;
			$xoopsDB->query( "INSERT INTO $table_text SET lid='$lid', description='$desc4save'" ) ;
			echo _MD_A_IELOG_MB_FINISHED."<br />\n" ;
...
$title_tray = new XoopsFormElementTray( _MD_A_IELOG_TH_TITLE , '<br /><br />' ) ;
$title_tray->addElement( $title_text ) ;
$title_tray->addElement( new XoopsFormLabel( "" , _MD_A_GNAV_BATCHBLANK ) ) ;

$cat = new XoopsFormSelect( _MD_A_IELOG_TH_CATEGORIES , "cid" , $cid ) ;
...
$submitter_select = new XoopsFormSelectUser( _MD_A_IELOG_TH_SUBMITTER , 'submitter' , false , $submitter ) ;

$date_text = new XoopsFormText( _MD_A_IELOG_TH_DATE , 'post_date' , 20 , 20 , formatTimestamp( time() , _MD_GNAV_DTFMT_YMDHI ) ) ;
...
$desc_tarea = new XoopsFormDhtmlTextarea( _MD_A_IELOG_TH_DESCRIPTION , 'desc' , $desc4edit , 10 , 50 ) ;
...
echo "<h3 style='text-align:left;'>".sprintf(_MD_A_IELOG_FMT_BATCHREGISTER,$xoopsModule->name())."</h3>\n" ;
ielog_opentable();
