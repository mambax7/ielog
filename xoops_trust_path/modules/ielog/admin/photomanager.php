...
if( ! empty( $_POST['action'] ) && $_POST['action'] == 'delete' && isset( $_POST['ids'] ) && is_array( $_POST['ids'] ) ) {
	...
	foreach( $_POST['ids'] as $lid ) {
		ielog_delete_photos( "lid=".intval( $lid ) ) ;
	}
...
foreach( $numbers as $number ) {
	...
	$num_options .= "<option value='$number' $selected>".sprintf(_MD_A_IELOG_FMT_PHOTONUM,$number)."</option>\n" ;
}

ielog_get_cat_options() ;

// Options for Selecting a category
$cat_options = ielog_get_cat_options( 'title' , $cid , '--' , '----' ) ;
$cat_options_for_update = ielog_get_cat_options( 'title' , 0 , '--' , _MD_A_IELOG_OPT_NOCHANGE ) ;

// Options for Selecting a user
$user_options = "<option value='0'>"._MD_A_IELOG_OPT_NOCHANGE."</option>\n" ;
...
echo "<h3 style='text-align:left;'>".sprintf(_MD_A_IELOG_FMT_PHOTOMANAGER,$xoopsModule->name())."</h3>\n" ;
  ...
  <table border='0' cellpadding='0' cellspacing='0' style='width:100%;'>
    <tr>
      <td align='left'>
        ...
        <input type='submit' value='"._MD_A_IELOG_BUTTON_EXTRACT."'> &nbsp; 
...
<p align='center' style='margin:0px;'>
  $photonavinfo
  <a href='../index.php?page=submitcid=$cid'><img src='../images/pictadd.gif' alt='"._MD_A_IELOG_CAT_LINK_ADDPHOTOS."' title='"._MD_A_IELOG_CAT_LINK_ADDPHOTOS."' /></a>
</p>
...
while( list( $lid , $title , $submitter , $ext , $w , $h , $status ) = $xoopsDB->fetchRow( $prs ) ) {
	...
	if( in_array( strtolower( $ext ) , $ielog_normal_exts ) ) {
		...
		$widthheight = $w > $h ? "width='$ielog_thumbsize'" : "height='$ielog_thumbsize'" ;
	} else {
		if($ext){
	...
	$deadlinkbutton = is_readable( "$photos_dir/{$lid}.{$ext}" ) ? "" : "<img src='".XOOPS_URL."/modules/$mydirname/images/deadlink.gif' border='0' alt='"._MD_A_IELOG_DEADLINKMAINPHOTO."' title='"._MD_A_IELOG_DEADLINKMAINPHOTO."' />" ;
	...
	echo "
		<td align='center' style='background-color:$bgcolor; margin: 0 px; padding: 3 px; border-width:0px 2px 2px 0px; border-style: solid; border-color:black;'>
			<table border='0' cellpadding='0' cellmargin='0'>
				<tr>
					<td></td>
					<td><img src='../images/pixel_trans.gif' width='$ielog_thumbsize' height='1' alt='' /></td>
					<td></td>
				</tr>
				<tr>
					<td><img src='../images/pixel_trans.gif' width='1' height='$ielog_thumbsize' alt='' /></td>
...
<tr>
	<td align='left'>
		<input type='button' value='"._MD_A_IELOG_BTN_SELECTNONE."' onclick=\"with(document.MainForm){for(i=0;i<length;i++){if(elements[i].type=='checkbox'){elements[i].checked=false;}}}\" />
		&nbsp;
		<input type='button' value='"._MD_A_IELOG_BTN_SELECTALL."' onclick=\"with(document.MainForm){for(i=0;i<length;i++){if(elements[i].type=='checkbox'){elements[i].checked=true;}}}\" />
	</td>
	<td align='right'>
		<input type='hidden' name='action' value='' />
		"._MD_A_IELOG_LABEL_REMOVE."<input type='button' value='"._MD_A_IELOG_BUTTON_REMOVE."' onclick='if(confirm(\""._MD_A_IELOG_JS_REMOVECONFIRM."\")){document.MainForm.action.value=\"delete\"; submit();}' />
	</td>
</tr>
</table>
<br />
<table class='outer' style='width:100%;'>
	<tr>
		<th colspan='2'>"._MD_A_IELOG_TH_BATCHUPDATE."</th>
	</tr>
	<tr>
		<td class='head'>"._MD_A_IELOG_TH_TITLE."</td>
		<td class='even'><input type='text' name='new_title' size='50' /></td>
	</tr>
	<tr valign='top'>
		<td class='head'>"._MD_A_IELOG_TH_DESCRIPTION."</td>
		<td class='even'><textarea name='new_desc_text' cols='50' rows='5'></textarea></td>
	</tr>
	<tr>
		<td class='head'>"._MD_A_IELOG_TH_CATEGORIES."</td>
		<td class='even'>
			<select name='new_cid'>
	...
	<tr>
		<td class='head'>"._MD_A_IELOG_TH_SUBMITTER."</td>
		<td class='even'>
	...
	<tr valign='top'>
		<td class='head'>"._MD_A_IELOG_TH_DATE."</td>
		<td class='even'><input type='text' name='new_post_date' size='20' value='".formatTimestamp(time(),_MD_GNAV_DTFMT_YMDHI)."'></textarea></td>
	</tr>
	<tr>
		<td class='head'></td>
		<td class='even'><input type='submit' name='update' value='"._MD_A_IELOG_BUTTON_UPDATE."' onclick='return confirm(\""._MD_A_IELOG_JS_UPDATECONFIRM."\")' tabindex='1' /></td>
	</tr>
</table>
</form>
" ;
