...
if(!$ielog_usegooglemap || !empty( $_POST['set_latlng'] )){
	$lat = 0 ;
	$lng = 0 ;
	$zoom = 0 ;
	$mtype = "" ;
	...
	if($lat==$ielog_defaultlat && $lng==$ielog_defaultlng){
		$lat = 0 ;
		$lng = 0 ;
		$zoom = 0 ;
		$mtype = "" ;
...
if( $action == "insert" ) {
	...
	redirect_header( "index.php?page=category" , 1 , _MD_A_IELOG_CAT_INSERTED ) ;
	exit ;
	...
	if( $pid != 0 ) {
		...
		foreach( $children as $child ) {
			if( $child == $pid ) redirect_header( "index.php?page=category&disp=edit&cid=$cid" , 1 , _MD_A_IELOG_CAT_LOOP ) ; 
		}
	}
	...
	redirect_header( "index.php?page=category" , 1 , _MD_A_IELOG_CAT_UPDATED ) ;
	exit ;
	...
	foreach( $children as $child ) {
		$whr .= "$child," ;
		xoops_notification_deletebyitem( $ielog_mid , 'category' , $child ) ;
	}
	$whr .= "$cid)" ;
	xoops_notification_deletebyitem( $ielog_mid , 'category' , $cid ) ;

	ielog_delete_photos( $whr ) ;

	$xoopsDB->query( "DELETE FROM $table_cat WHERE $whr" ) or die( "DB error: DELETE cat table" ) ;
	redirect_header( 'index.php?page=category' , 2 , _MD_A_IELOG_CATDELETED ) ;
	exit ;
...
echo "<h3 style='text-align:left;'>".sprintf( _MD_A_IELOG_FMT_CATEGORIES , $xoopsModule->name() )."</h3>\n" ;
...
if( $disp == "edit" && $cid > 0 ) {
	...
	display_edit_form( $cat_array , _MD_A_IELOG_CAT_MENU_EDIT , 'update' ) ;

} else if( $disp == "new" ) {

	// New
	$cat_array = array( 'cid' => 0 , 'pid' => $cid , 'title' => '' , 'imgurl' => 'http://','kmlurl' => 'http://', 'description' => '','weight' => 0,'lat'=> 0,'lng'=> 0,'zoom'=> 0 ,'mtype'=>'' ,'icd'=> 0 ) ;
	display_edit_form( $cat_array , _MD_A_IELOG_CAT_MENU_NEW , 'insert' ) ;
	...
	$link_admission = $waiting > 0 ? sprintf( _MD_A_IELOG_CAT_FMT_NEEDADMISSION , $waiting ) : '' ;

	// Top links
	echo "<p><a href='?page=category&disp=new&cid=0'>"._MD_A_IELOG_CAT_LINK_MAKETOPCAT."</a> &nbsp;  &nbsp; <a href='index.php?page=admission' style='color:red;'>$link_admission</a></p>\n" ;
	...
	<table width='75%' class='outer' cellpadding='4' cellspacing='1'>
	  <tr valign='middle'>
	    <th nowrap='nowrap'>"._MD_A_IELOG_CAT_TH_IMAGE."</th>
	    <th>"._MD_A_IELOG_CAT_TH_TITLE."</th>
	    <th>"._MD_A_IELOG_CAT_TH_WEIGHT."</th>

	    <th>"._MD_A_IELOG_CAT_TH_PHOTOS."</th>
	    <th>"._MD_A_IELOG_CAT_TH_OPERATION."</th>
	  </tr>
	" ;
	...
	foreach( $cat_tree_array as $cat_node ) {
		...
		$cid = intval( $cid ) ;
		$del_confirm = 'confirm("' . sprintf( _MD_A_IELOG_CAT_FMT_CATDELCONFIRM , $title ) . '")' ;
	  ...
	  <tr>
	    ...
	    <td class='$oddeven' nowrap='nowrap' align='right'>
	      <a href='index.php?page=photomanager&cid=$cid'>$photos_num</a>&nbsp;"._MD_A_IELOG_C_DATA."&nbsp;&nbsp;
	      [<a href='../index.php?page=submit&cid=$cid'>"._MD_A_IELOG_CAT_LINK_ADDPHOTOS."</a>]</td>
	    <td class='$oddeven' align='center' nowrap='nowrap'>
	      &nbsp;
	      [<a href='?page=category&disp=edit&amp;cid=$cid'>"._MD_A_IELOG_CAT_LINK_EDIT."</a>]
	      &nbsp;
	      [<a href='?page=category&disp=new&amp;cid=$cid'>"._MD_A_IELOG_CAT_LINK_MAKESUBCAT."</a>]
	      &nbsp;
	...
	echo "
	  <!-- <tr>
	    <td colspan='4' align='right' class='foot'><input type='submit' name='batch_update' value='"._MD_A_IELOG_CAT_BTN_BATCH."' /></td>
	  </tr> -->
	</table>
	</form>
	" ;
...
function display_edit_form( $cat_array , $form_title , $action)
{
	...
	global $ielog_defaultlat;
	global $ielog_defaultlng;
	global $ielog_defaultzoom;
	global $ielog_defaultmtype;
	global $ielog_googlemapapi_key;
	global $ielog_usegooglemap,$xoopsDB;
	global $ielog_lang_java;
	global $ielog_body_editor;
	global $ielog_googlemap_url;
	...
	// Title
	$form->addElement( new XoopsFormText( _MD_A_IELOG_CAT_TH_TITLE , 'title' , 30 , 50 , $myts->htmlSpecialChars( $title ) ) , true ) ;

	// Image URL
	$form->addElement( new XoopsFormText( _MD_A_IELOG_CAT_TH_IMGURL , 'imgurl' , 50 , 150 , $myts->htmlSpecialChars( $imgurl ) ) ) ;

	// Kml URL
	$form->addElement( new XoopsFormText( _MD_A_IELOG_CAT_TH_KMLURL , 'kmlurl' , 50 , 150 , $myts->htmlSpecialChars( $kmlurl ) ) ) ;
	...
	ob_end_clean() ;
	$form->addElement( new XoopsFormLabel( _MD_A_IELOG_CAT_TH_PARENT , $cat_selbox ) ) ;

//----------------------------editor-------------------------------------------
	if( $ielog_body_editor == 'common_fckeditor') {
		...
		$desc_tarea =  new XoopsFormLabel( _MD_A_IELOG_CAT_TH_DESC , $jscript.$wysiwyg_body ) ;
	} else if( $ielog_body_editor == 'common_spaw' && file_exists( XOOPS_ROOT_PATH.'/common/spaw/spaw_control.class.php' ) ) {
		...
		ob_end_clean() ;
		$desc_tarea =  new XoopsFormLabel( _MD_A_IELOG_CAT_TH_DESC , $wysiwyg_body ) ;
	}else if ($ielog_body_editor == 'pure_html'){
		$desc_tarea = new XoopsFormTextArea(_MD_A_IELOG_CAT_TH_DESC, "description" , $myts->makeTareaData4Edit( $description ) , 20 , 60 ) ;
	} else {
		$desc_tarea = new XoopsFormDhtmlTextArea(_MD_A_IELOG_CAT_TH_DESC, "description" , $myts->makeTareaData4Edit( $description ) , 20 , 60 ) ;
	}
...
//---------------------------------------------------------------------------------
	...
	$form->addElement( new XoopsFormText(_MD_A_IELOG_CAT_TH_WEIGHT, 'weight' , 5 , 3 , $myts->htmlSpecialChars( $weight ) ) ) ;
...
$set_latlng_state="";
if($lat==0 && $lng==0){
	$lat=$ielog_defaultlat;
	$lng=$ielog_defaultlng;
	$zoom=$ielog_defaultzoom;
	$mtype=$ielog_defaultmtype;
	if( $action == "update") {
...
<script src='".$ielog_googlemap_url."/maps?file=api&amp;v=2&amp;key=$ielog_googlemapapi_key' type='text/javascript' charset='utf-8'></script>
