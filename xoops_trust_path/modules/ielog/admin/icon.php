...
$title = isset( $_POST[ 'title' ] ) ? $myts->stripSlashesGPC( @$_POST["title"] ):'' ;
	...
	list($tmp_name1,$shadow_ext,$errmsg) = ielog_submit_uploader(@$_POST["xoops_upload_file"][1] ,$del_photo1,$preview_name1, 2, $errmsg);

	if($ext=='' && empty( $ielog_allownoimage ) ) {
	...
	if(( $ext && !in_array( strtolower( $ext ) , $ielog_normal_exts)) ||($shadow_ext && !in_array( strtolower( $shadow_ext ) , $ielog_normal_exts ))) {
		if($tmp_name)@unlink($tmp_name) ;
		if($tmp_name1)@unlink($tmp_name1) ;
		redirect_header( 'index.php?page=icon' , 2 , "allowed file type is '".implode( "','" , $ielog_normal_exts )."'" ) ;
		exit ;
	}
	...
	if($ext){
		ielog_modify_photo( "$photos_dir/$tmp_name" , "$icon_dir/$icd.$ext" ) ;
	...
	if($shadow_ext){
		ielog_modify_photo( "$photos_dir/$tmp_name1" , $icon_dir."/".$icd."_s.".$shadow_ext ) ;
	}else{
	...
	if( $action == "insert" ) {
		redirect_header( "index.php?page=icon" , 1 , _MD_A_IELOG_ICO_INSERTED ) ;
	} else{
		redirect_header( "index.php?page=icon" , 1 , _MD_A_IELOG_ICO_UPDATED ) ;
	}
	exit ;
	...
	redirect_header( 'index.php?page=icon' , 2 , _MD_A_IELOG_ICODELETED ) ;
	exit ;
...
echo "<h3 style='text-align:left;'>".sprintf( _MD_A_IELOG_FMT_ICONS , $xoopsModule->name() )."</h3>\n" ;
...
if( $disp == "edit" && $icd > 0 ) {
	...
	display_edit_form( $icons , _MD_A_IELOG_CAT_MENU_EDIT , 'update' ) ;

} else if( $disp == "new" ) {

	// New
	$icons = array( 'icd' => 0 , 'title' => '' ,'ext' => '' , 'shadow_ext' => '' , 'Anchor_x' => 0,'Anchor_y' => 0,'infoWindowAnchor_x' => 0,'infoWindowAnchor_y' => 0 ) ;
	display_edit_form( $icons , _MD_A_IELOG_CAT_MENU_NEW , 'insert' ) ;

} else {
	// Top links
	echo "<p><a href='?page=icon&disp=new&icd=0'>"._MD_A_IELOG_CAT_LINK_MAKEICO."</a></p>\n" ;
	...
	<table width='75%' class='outer' cellpadding='4' cellspacing='1'>
	  <tr valign='middle'>
	    <th>"._MD_A_IELOG_ICO_LIST_NO."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_TITLE."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_ICON."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_SHADOW."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_ANCHOR."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_WINANC."</th>
	    <th>"._MD_A_IELOG_ICO_LIST_EDIT."</th>
	  </tr>
	" ;
	...
	while( $icon = $xoopsDB->fetchArray( $crs ) ) {
		...
		extract( $icon ) ;
		$del_confirm = 'confirm("' . sprintf( _MD_A_IELOG_ICO_FMT_CATDELCONFIRM , $title ) . '")' ;
		echo "
	  <tr>
	    ...
	    <td class='$oddeven' style='vertical-align:middle;' align='center'>
	      &nbsp;
	      [<a href='?page=icon&disp=edit&amp;icd=$icd'>"._MD_A_IELOG_CAT_LINK_EDIT."</a>]
	      &nbsp;
	...
	echo "
	  <!-- <tr>
	    <td colspan='6' align='right' class='foot'><input type='submit' name='batch_update' value='"._MD_A_IELOG_CAT_BTN_BATCH."' /></td>
	  </tr> -->
	</table>
	</form>
	" ;
...
function display_edit_form( $icons , $form_title , $action)
{
	...
	global $ielog_usegooglemap,$ielog_fsize;
	...
	// Title
	$form->addElement( new XoopsFormText( _MD_A_IELOG_ICO_TH_TITLE , 'title' , 30 , 50 , $myts->htmlSpecialChars( $title ) ) , true ) ;

	if($ext){
		$photoview = new XoopsFormLabel(_MD_A_IELOG_ICO_IMG, "<img src='".$icon_url."/".$icd.".".$ext."' />" ) ;
		$file_form = new XoopsFormFile(_MD_A_IELOG_ICO_EDIIMG, "photofile" , $ielog_fsize ) ;
	}else{
		$file_form = new XoopsFormFile(_MD_A_IELOG_ICO_IMG, "photofile" , $ielog_fsize ) ;
		$form->setRequired( $file_form ) ;
	}
	$file_form->setExtra( "size='70'" ) ;
	if($shadow_ext){
		$photoview1 = new XoopsFormLabel(_MD_A_IELOG_ICO_SHADOW, "<img src='".$icon_url."/".$icd."_s.".$shadow_ext."'  />" ) ;
		$file_form1 = new XoopsFormFile(_MD_A_IELOG_ICO_EDISHADOW, "photofile1" , $ielog_fsize ) ;
	}else{
		$file_form1 = new XoopsFormFile(_MD_A_IELOG_ICO_SHADOW, "photofile1" , $ielog_fsize ) ;
	}
	$file_form1->setExtra( "size='70'" ) ;
	$del_box1 = new XoopsFormCheckBox( "&nbsp;" , "del_photo1" , array( 0 ) ) ;
	$del_box1->addOption( '1' ,_MD_A_IELOG_ICO_DELSHADOW) ;
