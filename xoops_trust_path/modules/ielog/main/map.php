...
$default_lat  = empty( $_GET['lat'] )  ? $ielog_defaultlat  : floatval( $_GET['lat'] ) ;
$default_lng  = empty( $_GET['lng'] )  ? $ielog_defaultlng  : floatval( $_GET['lng'] ) ;
$default_zoom = empty( $_GET['zoom'] ) ? $ielog_defaultzoom : intval( $_GET['zoom'] ) ;
$default_mtype = !in_array(@$_GET['mtype'],$ielog_maptypes) ? $ielog_defaultmtype : $_GET['mtype'] ;
$default_mtype = !in_array(@$_GET['mtype'],$ielog_maptypes) ? $ielog_defaultmtype : $_GET['mtype'] ;
...
if( $cid > 0 ) {
	...
	$xoopsTpl->assign( 'subcategories' , ielog_get_sub_categories( $cid , $cattree,"AND (lat<>0 OR lng<>0)" ) ) ;
	...
	//counts
	$photo_num_total = ielog_get_photo_total_sum_from_cats( $cids , "status>0 AND (lat<>0 OR lng<>0)" ) ;
	...
	$xoops_breadcrumbs = ielog_add_breadcrumbs( $pid , "index.php".( $page_map ? '?'.$page_map:''),$xoops_breadcrumbs);
	...
	$xoopsTpl->assign( 'subcategories' , ielog_get_sub_categories( 0 , $cattree,"AND (lat<>0 OR lng<>0)" ) ) ;
	$xoopsTpl->assign( 'category_options' , ielog_get_cat_options() ) ;
...
//<![CDATA[
	...
	gn_drkm = $ielog_map_draw ;
	window.onload = ShowGMap;
	".($ielog_pe_appkey==""?"":"gn_pekey='".$ielog_pe_appkey."';")."
//]]>
</script>";
...
//for mobile code 
	...
	//for degug
		...
		$mymap="$google_staticmap?center=$default_lat,$default_lng&zoom=$default_zoom&size=$ielog_mobile_mapsize&maptype=$ielog_mobile_maptype&key=$ielog_googlemapapi_key";

		$ielog_mobile_maekercolor="blue";

		/*緯度は -90度 〜 +90度の範囲に、経度は -180度 〜 +180度の範囲に収まるように*/

		$movex=$ielog_mobile_mapsizex/pow(2,$default_zoom);
		$movey=$ielog_mobile_mapsizey/pow(2,$default_zoom);

		//Amount of movement on Mini map . set bigger than 0 and 1 or less. (0 < value <=1).
		$ielog_mobile_mapmove_raito = 0.6;  

		$latup = ielog_latlnground($default_lat+$movey * $ielog_mobile_mapmove_raito);
		$latdown = ielog_latlnground($default_lat-$movey * $ielog_mobile_mapmove_raito);
		$lngup = ielog_latlnground($default_lng+$movex * $ielog_mobile_mapmove_raito);
		$lngdown = ielog_latlnground($default_lng-$movex * $ielog_mobile_mapmove_raito);
		...
		$latup = ielog_latlnground($default_lat+$movey/2);
		$latdown = ielog_latlnground($default_lat-$movey/2);
		$lngup = ielog_latlnground($default_lng+$movex/2);
		$lngdown = ielog_latlnground($default_lng-$movex/2);
		...
		while( $rs = $xoopsDB->fetchArray( $prs ) ) {
			...
			$markers.=$rs['lat'].",".$rs['lng'].",".$ielog_mobile_maekercolor.$abc;
		...
		ielog_mobile_templete_disp("db:{$mydirname}_mobile_map.html");
...
function ielog_latlnground($value){
...
function address2latlng( $address ){
    ...
    $url = $ielog_googlemap_url."/maps/geo?key=".$ielog_googlemapapi_key."&q=".$address."&output=xml&ie=UTF-8";
