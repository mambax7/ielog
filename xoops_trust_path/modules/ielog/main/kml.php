...
while( $rs = $xoopsDB->fetchArray( $prs ) ) {
	...
	if($ext){
		if( in_array( strtolower( $ext ) , $ielog_normal_exts ) ) {
			...
			if($ielog_thumbrule=='w' || ($ielog_thumbrule=='b' && $rs['res_x'] > $rs['res_y'] )){
				if($rs['res_x']>$ielog_thumbsize){
					if($rs['res_x']>0)$y=intval($ielog_thumbsize/$rs['res_x']*$rs['res_y']);
				}else{
					$y=$rs['res_y'];
				}
			}else{
				if($rs['res_y']>$ielog_thumbsize){
					$y=$ielog_thumbsize;
				}else{
					$y=$rs['res_y'];
				}
	...
	$icon = $rs['icd'] > 0 ? $rs['icd'] : ielog_get_icon($cattree,$rs['cid'],$rs['cid1'],$rs['cid2'],$rs['cid3'],$rs['cid4'],$cid);
	if($icon>0){
		$iconstyle='
