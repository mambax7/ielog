...
function ielog_get_thumbnail_wh( $width , $height )
{
	global $ielog_thumbsize , $ielog_thumbrule ;

	switch( $ielog_thumbrule ) {
		case 'w' :
			$new_w = $ielog_thumbsize ;
			$scale = $width / $new_w ;
			$new_h = intval( round( $height / $scale ) ) ;
			break ;
		case 'h' :
			$new_h = $ielog_thumbsize ;
			$scale = $height / $new_h ;
...
function ielog_create_thumb( $src_path , $node , $ext )
{
	global $ielog_imagingpipe , $ielog_makethumb , $ielog_normal_exts ;

	if( ! in_array( strtolower( $ext ) , $ielog_normal_exts ) ) {
		return ielog_copy_thumb_from_icons( $src_path , $node , $ext ) ;
	}

	if( ! $ielog_makethumb ) return 3 ;

	if( $ielog_imagingpipe == PIPEID_IMAGICK ) {
		return ielog_create_thumb_by_imagick( $src_path , $node , $ext ) ;
	} else if( $ielog_imagingpipe == PIPEID_NETPBM ) {
		return ielog_create_thumb_by_netpbm( $src_path , $node , $ext ) ;
	} else {
		return ielog_create_thumb_by_gd( $src_path , $node, $ext ) ;
	}
}


// Copy Thumbnail from directory of icons
function ielog_copy_thumb_from_icons( $src_path , $node , $ext )
{
...
function ielog_create_thumb_by_gd( $src_path , $node , $ext )
{
	global $ielog_forcegd2 , $thumbs_dir ;

	$bundled_2 = false ;
	if( ! $ielog_forcegd2 && function_exists( 'gd_info' ) ) {
		$gd_info = gd_info() ;
	...
	list( $new_w , $new_h ) = ielog_get_thumbnail_wh( $width , $height ) ;
...
function ielog_create_thumb_by_imagick( $src_path , $node , $ext )
{
	global $ielog_imagickpath , $thumbs_dir ;

	// Check the path to binaries of imaging packages
	if( trim( $ielog_imagickpath ) != '' && substr( $ielog_imagickpath , -1 ) != DIRECTORY_SEPARATOR ) {
		$ielog_imagickpath .= DIRECTORY_SEPARATOR ;
	}
	...
	exec( "{$ielog_imagickpath}convert -geometry {$new_w}x{$new_h} $src_path $thumbs_dir/$node.$ext" ) ;
...
function ielog_create_thumb_by_netpbm( $src_path , $node , $ext )
{
	global $ielog_netpbmpath , $thumbs_dir ;

	// Check the path to binaries of imaging packages
	if( trim( $ielog_netpbmpath ) != '' && substr( $ielog_netpbmpath , -1 ) != DIRECTORY_SEPARATOR ) {
		$ielog_netpbmpath .= DIRECTORY_SEPARATOR ;
	}
	...
	switch( $type ) {
		case 1 :
			// GIF
			$pipe0 = "{$ielog_netpbmpath}giftopnm" ;
			$pipe2 = "{$ielog_netpbmpath}ppmquant 256 | {$ielog_netpbmpath}ppmtogif" ;
			break ;
		case 2 :
			// JPEG
			$pipe0 = "{$ielog_netpbmpath}jpegtopnm" ;
			$pipe2 = "{$ielog_netpbmpath}pnmtojpeg" ;
			break ;
		case 3 :
			// PNG
			$pipe0 = "{$ielog_netpbmpath}pngtopnm" ;
			$pipe2 = "{$ielog_netpbmpath}pnmtopng" ;
			break ;
		default :
			@copy( $src_path , "$thumbs_dir/$node.$ext" ) ;
			return 2 ;
	...
	$pipe1 = "{$ielog_netpbmpath}pnmscale -xysize $new_w $new_h" ;
...
// modifyPhoto Wrapper
function ielog_modify_photo( $src_path , $dst_path )
{
	global $ielog_imagingpipe , $ielog_forcegd2 , $ielog_normal_exts ;
	...
	if( $ielog_imagingpipe == PIPEID_IMAGICK ) {
		ielog_modify_photo_by_imagick( $src_path , $dst_path ) ;
	} else if( $ielog_imagingpipe == PIPEID_NETPBM ) {
		ielog_modify_photo_by_netpbm( $src_path , $dst_path ) ;
	} else {
		if( $ielog_forcegd2 ) ielog_modify_photo_by_gd( $src_path , $dst_path ) ;
...
function ielog_modify_photo_by_gd( $src_path , $dst_path )
{
	...
	if( $width > $ielog_width || $height > $ielog_height ) {
		if( $width / $ielog_width > $height / $ielog_height ) {
			$new_w = $ielog_width ;
			$scale = $width / $new_w ; 
			$new_h = intval( round( $height / $scale ) ) ;
...
function ielog_modify_photo_by_imagick( $src_path , $dst_path )
{
	global $ielog_width , $ielog_height , $ielog_imagickpath ;
	...
	if( $image_stats[0] > $ielog_width || $image_stats[1] > $ielog_height ) {
		$option .= " -geometry {$ielog_width}x{$ielog_height}" ;
	}
	...
	if( $option != "" ) exec( "{$ielog_imagickpath}convert $option $src_path $dst_path" ) ;
	...
	if( file_exists( $wmfile ) ) {
		exec( "{$ielog_imagickpath}composite -compose plus $wmfile $dst_path $dst_path" ) ;
	}
...
function ielog_modify_photo_by_netpbm( $src_path , $dst_path )
{
	global $ielog_width , $ielog_height , $ielog_netpbmpath ;
	...
	if( isset( $_POST['rotate'] ) ) switch( $_POST['rotate'] ) {
		case 'rot270' :
			$pipe1 .= "{$ielog_netpbmpath}pnmflip -r90 |" ;
			break ;
		case 'rot180' :
			$pipe1 .= "{$ielog_netpbmpath}pnmflip -r180 |" ;
			break ;
		case 'rot90' :
			$pipe1 .= "{$ielog_netpbmpath}pnmflip -r270 |" ;
			break ;
		default :
		case 'rot0' :
			break ;
...
function ielog_clear_tmp_files( $dir_path , $prefix = 'tmp_' )
{
...
function ielog_updaterating( $lid )
{
...
function ielog_get_photo_small_sum_from_cat( $cid , $whr_append = "" )
{
...
function ielog_get_photo_total_sum_from_cats( $cids , $whr_append = "" )
{
	global $xoopsDB , $table_photos ;
...
// Update a photo 
function ielog_update_item($mode,$lid,
	...
	if( $mode == G_INSERT){
		...
		$sql .= "url,tel,fax,zip,other1,other2,other3,other4,other5,other6,other7,other8,other9,other10,other11,other12,other13,other14,other15,other16,other17,other18,other19,other20,address,rss,lat,lng,zoom,mtype,icd,";
		$sql .= "submitter,poster_name,status,date) VALUES (";
		$sql .= "$lid,'".addSlashes($title)."',$cid,$cid1,$cid2,$cid3,$cid4,";
		$sql .= "'".addSlashes($url)."','".addSlashes($tel)."','".addSlashes($fax)."','".addSlashes($zip)."','".addSlashes($other1)."','".addSlashes($other2)."','".addSlashes($other3)."','".addSlashes($other4)."','".addSlashes($other5)."','".addSlashes($other6)."','".addSlashes($other7)."','".addSlashes($other8)."','".addSlashes($other9)."','".addSlashes($other10)."','".addSlashes($other11)."','".addSlashes($other12)."','".addSlashes($other13)."','".addSlashes($other14)."','".addSlashes($other15)."','".addSlashes($other16)."','".addSlashes($other17)."','".addSlashes($other18)."','".addSlashes($other19)."','".addSlashes($other20)."','".addSlashes($address)."','".addSlashes($rss)."',$lat,$lng,$zoom,'".addSlashes($mtype)."',$icd,";
        ...
        $sql .= "url='".addSlashes($url)."',tel='".addSlashes($tel)."',fax='".addSlashes($fax)."',zip='".addSlashes($zip)."',other1='".addSlashes($other1)."',other2='".addSlashes($other2)."',other3='".addSlashes($other3)."',other4='".addSlashes($other4)."',other5='".addSlashes($other5)."',other6='".addSlashes($other6)."',other7='".addSlashes($other7)."',other8='".addSlashes($other8)."',other9='".addSlashes($other9)."',other10='".addSlashes($other10)."',other11='".addSlashes($other11)."',other12='".addSlashes($other12)."',other13='".addSlashes($other13)."',other14='".addSlashes($other14)."',other15='".addSlashes($other15)."',other16='".addSlashes($other16)."',other17='".addSlashes($other17)."',other18='".addSlashes($other18)."',other19='".addSlashes($other19)."',other20='".addSlashes($other20)."',address='".addSlashes($address)."',rss='".addSlashes($rss)."',lat=$lat,lng=$lng,zoom=$zoom,mtype='".addSlashes($mtype)."',icd=$icd,";
...
function ielog_update_desc($mode,$lid,$cid,$title,$submitter,$valid,
	...
	$caption3 = $ext3=='' ? '' : $caption3 ;
	$caption4 = $ext4=='' ? '' : $caption4 ;
	$caption5 = $ext5=='' ? '' : $caption5 ;
	$caption6 = $ext6=='' ? '' : $caption6 ;
	$caption7 = $ext7=='' ? '' : $caption7 ;
	$caption8 = $ext8=='' ? '' : $caption8 ;
	
	$sql="UPDATE $table_photos SET caption='".addSlashes($caption)."',caption1='".addSlashes($caption1)."',caption2='".addSlashes($caption2)."',caption3='".addSlashes($caption3)."',caption4='".addSlashes($caption4)."',caption5='".addSlashes($caption5)."',caption6='".addSlashes($caption6)."',caption7='".addSlashes($caption7)."',caption8='".addSlashes($caption8)."',ext='{$ext}',ext1='{$ext1}',ext2='{$ext2}',ext3='{$ext3}',ext4='{$ext4}',ext5='{$ext5}',ext6='{$ext6}',ext7='{$ext7}',ext8='{$ext8}',res_x={$resx},res_y={$resy},res_x1={$resx1},res_y1={$resy1},res_x2={$resx2},res_y2={$resy2},res_x3={$resx3},res_y3={$resy3},res_x4={$resx4},res_y4={$resy4},res_x5={$resx5},res_y5={$resy5},res_x6={$resx6},res_y6={$resy6},res_x7={$resx7},res_y7={$resy7},res_x8={$resx8},res_y8={$resy8} WHERE lid='$lid'" ;
	...
	if( $valid == 1 ) {
		...
		// Global Notification
		ielog_trigger_event('global', 0, 'new_item', $tags);
		...
		ielog_trigger_event('category', $cid, 'new_item', $tags);

	}

}

function ielog_trigger_event( $category , $item_id , $event , $tags, $extra_tags=array() , $user_list=array() , $omit_user_id=null )
{
...
function ielog_delete_photos( $whr )
{
	global $xoopsDB ;
	global $photos_dir , $thumbs_dir , $ielog_mid ;
	global $table_photos , $table_text , $table_votedata ;

	$prs = $xoopsDB->query("SELECT lid, ext, ext1, ext2, ext3, ext4, ext5, ext6, ext7, ext8 FROM $table_photos WHERE $whr" ) ;
	while( list( $lid , $ext , $ext1 , $ext2 , $ext3 , $ext4 , $ext5 , $ext6 , $ext7 , $ext8 ) = $xoopsDB->fetchRow( $prs ) ) {

		xoops_comment_delete( $ielog_mid , $lid ) ;
		xoops_notification_deletebyitem( $ielog_mid , 'photo' , $lid ) ;
		...
		if($ext3){
			@unlink( $photos_dir."/".$lid."_3.".$ext3 ) ;
			@unlink( $photos_dir."/".$lid."_3.".$ext3 ) ;
		}
		if($ext4){
			@unlink( $photos_dir."/".$lid."_4.".$ext4 ) ;
			@unlink( $photos_dir."/".$lid."_4.".$ext4 ) ;
		}
		if($ext5){
			@unlink( $photos_dir."/".$lid."_5.".$ext5 ) ;
			@unlink( $photos_dir."/".$lid."_5.".$ext5 ) ;
		}
		if($ext6){
			@unlink( $photos_dir."/".$lid."_6.".$ext6 ) ;
			@unlink( $photos_dir."/".$lid."_6.".$ext6 ) ;
		}
		if($ext7){
			@unlink( $photos_dir."/".$lid."_7.".$ext7 ) ;
			@unlink( $photos_dir."/".$lid."_7.".$ext7 ) ;
		}
		if($ext8){
			@unlink( $photos_dir."/".$lid."_8.".$ext8 ) ;
			@unlink( $photos_dir."/".$lid."_8.".$ext8 ) ;
		}
...
function ielog_get_cat_options( $order = 'title' , $preset = 0 , $prefix = '--' , $none = null , $table_name_cat = null , $table_name_photos = null )
{
	global $xoopsDB ;
...
function ielog_submit_uploader_pre($field , $preview_name,$del_photo,$guard_name){

	global $ielog_canresize,$photos_dir , $array_allowed_mimetypes , $ielog_fsize , $ielog_width , $ielog_height , $array_allowed_exts;
	...
	if( is_readable( $_FILES[$field]['tmp_name'] ) ) {
		...
		if( $ielog_canresize ) $uploader = new MyXoopsMediaUploader( $photos_dir , $array_allowed_mimetypes , $ielog_fsize , null , null , $array_allowed_exts ) ;
		else $uploader = new MyXoopsMediaUploader( $photos_dir , $array_allowed_mimetypes , $ielog_fsize , $ielog_width , $ielog_height , $array_allowed_exts ) ;
		$uploader->setPrefix( 'tmp_' ) ;
		if( $uploader->fetchMedia( $field ) && $uploader->upload() ) {
			$tmp_name = $uploader->getSavedFileName() ;
			$preview_name = str_replace( 'tmp_' , 'tmp_prev_' , $tmp_name ) ;
			ielog_modify_photo( "$photos_dir/$tmp_name" , "$photos_dir/$preview_name" ) ;
		} else {
...
function ielog_submit_uploader($field ,$del_photo,$preview_name, $num, $errmsg){
...
function ielog_get_icon($cattree,$cid,$cid1,$cid2,$cid3,$cid4,$mcid=0){
	if($mcid>0){
		//$cid
		if(in_array($mcid,$cattree->getAllParentId($cid))){
			$icon=ielog_geticon_byParentId($cid);
			if($icon>0)return $icon;
		}
		//$cid1
		if(in_array($mcid,$cattree->getAllParentId($cid1))){
			$icon=ielog_geticon_byParentId($cid1);
			if($icon>0)return $icon;
		}
		//$cid2
		if(in_array($mcid,$cattree->getAllParentId($cid2))){
			$icon=ielog_geticon_byParentId($cid2);
			if($icon>0)return $icon;
		}
		//$cid3
		if(in_array($mcid,$cattree->getAllParentId($cid3))){
			$icon=ielog_geticon_byParentId($cid3);
			if($icon>0)return $icon;
		}
		//$cid4
		if(in_array($mcid,$cattree->getAllParentId($cid4))){
			$icon=ielog_geticon_byParentId($cid4);
			if($icon>0)return $icon;
		}
	}
...
function ielog_geticon_byParentId($cid){
	...
	return ielog_geticon_byParentId(intval($pid));
}

function ielog_get_anony_perms(){
	global $xoopsDB,$mydirname ;
	$anony_perms=0;
	$whr_groupid = "GPERM_groupid=".XOOPS_GROUP_ANONYMOUS ;
	$rs = $xoopsDB->query( "SELECT GPERM_itemid FROM ".$xoopsDB->prefix("group_permission")." LEFT JOIN ".$xoopsDB->prefix("modules")." m ON GPERM_modid=m.mid WHERE m.dirname='$mydirname' AND GPERM_name='ielog_global' AND ($whr_groupid)" ) ;
	while( list( $itemid ) = $xoopsDB->fetchRow( $rs ) ) {
		$anony_perms |= $itemid ;
	}
	return $anony_perms;
}
function ielog_get_gicon($icd,$op=0){
...
function ielog_addinfo_reg($str){
	$str=trim($str);
...
function ielog_addinfo_array($str,$myts){
	$str=ielog_addinfo_reg($str);
	$strarray=explode("<",$str);
...
function ielog_check_folders(){

	global $photos_dir,$mydirname,$ielog_makethumb,$thumbs_dir,$qrimg_dir,$ielog_mobile_useqr;
	...
	if( $ielog_mobile_useqr && ! is_dir( $qrimg_dir ) ) {
		if( $safe_mode_flag ) {
	...
	if( $ielog_makethumb && ! is_writable( $thumbs_dir ) ) {
	...
	if( $ielog_mobile_useqr && ! is_writable( $qrimg_dir ) ) {
...
function ielog_add_breadcrumbs($sel_id, $url , $arrcrumbs ){

	$arr = ielog_make_breadcrumbs($sel_id, $url);
	foreach($arr as $a){
		$arrcrumbs[]=$a;
	}
	return $arrcrumbs;
}
function ielog_make_breadcrumbs($sel_id, $url , $arrcrumbs = array() )
{
	...
	$arrcrumbs = ielog_make_breadcrumbs( $pid , $url , $arrcrumbs );
...
function ielog_mobile_templete_disp($templete){
	global $xoopsTpl,$ielog_mobile_encording;
	
	$out_text='';

    if (XOOPS_USE_MULTIBYTES == 1 && $ielog_mobile_encording && $ielog_mobile_encording!=_CHARSET) {
        if (function_exists('mb_convert_encoding')) {

			$xoopsTpl->assign('charset',$ielog_mobile_encording);
			$buffer = $xoopsTpl->fetch($templete);
            $out_text = mb_convert_encoding($buffer, $ielog_mobile_encording , _CHARSET);
			$charset=$ielog_mobile_encording;
        }
    }
