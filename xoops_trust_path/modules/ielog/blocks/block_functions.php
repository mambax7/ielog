...
function b_ielog_ritem_show( $options )
{
	...
	return b_ielog_itemblock_show($options,$this_template,$query);
}

function b_ielog_ritem_edit( $options )
{
	return b_ielog_itemblock_edit( $options );
}

function b_ielog_topnews_show( $options )
{
	$mydirname = empty( $options[0] ) ? basename( dirname( dirname( __FILE__ ) ) ) : $options[0] ;
	$show_image = empty( $options[7] ) ? 0 : 1 ;
...
function b_ielog_topnews_edit( $options )
{
	return b_ielog_itemblock_edit( $options );
}

function b_ielog_tophits_show( $options )
{
	$mydirname = empty( $options[0] ) ? basename( dirname( dirname( __FILE__ ) ) ) : $options[0] ;
	$show_image = empty( $options[7] ) ? 0 : 1 ;
...
function b_ielog_tophits_edit( $options )
{
	return b_ielog_itemblock_edit( $options );
}


function b_ielog_itemblock_show( $options ,$this_template,$query)
{
	global $xoopsDB ;
	...
	if(!$show_noimage){
		$whr_ext = "l.ext IN ('" . implode( "','" , $ielog_normal_exts ) . "')" ;
	} else {
		$whr_ext = "1" ;
	}
	...
	while( $photo = $xoopsDB->fetchArray( $result ) ) {
		...
		$photo['is_newphoto'] = ( $photo['unixtime'] > time() - 86400 * $ielog_newdays && $photo['status'] == 1 );
		$photo['is_updatedphoto'] = ( $photo['unixtime'] > time() - 86400 * $ielog_newdays && $photo['status'] == 2 );
		$photo['is_popularphoto'] = ( $photo['hits'] >= $ielog_popular );
		...
		if( in_array( strtolower( $photo['ext'] ) , $ielog_normal_exts ) ) {
			...
			if( $box_size <= 0 ) {
				...
				if( $photo['res_x'] > $box_size || $photo['res_y'] > $box_size && $ielog_thumbsize > $box_size ) {
...
function b_ielog_itemblock_edit( $options )
{
	global $xoopsDB ;
